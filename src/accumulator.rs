...
use halo2_proofs::{
    arithmetic::FieldExt,
    circuit::{Layouter, Region, SimpleFloorPlanner, AssignedCell},
...
impl<F: FieldExt> Circuit<F> for MyCircuit<F> {
    ...
    fn synthesize(
        ...
        let global_score = layouter.assign_region(|| "accumulator", |mut region: Region<'_, F>| {
			...
			for i in 0..self.opinions.len() {
				if let Some(opinion) = &self.opinions[i] {
					...
					let sum_cell = if let Some(accumulated_cell) = accumulated_cell {
						accumulated_cell.copy_advice(|| "acc", &mut region, config.advices[0], i)?
					} else {
						region.assign_advice(|| "acc", config.advices[0], i, || Ok(F::zero()))?
					};

					let lhs_cell = region.assign_advice(|| "lhs", config.advices[1], i, || Ok(opinion.local_score))?;
					let rhs_cell = region.assign_advice(|| "rhs", config.advices[2], i, || Ok(opinion.global_score))?;

					let value = lhs_cell.value().and_then(|&a| rhs_cell.value().map(|&b| a * b));
					let out_score = sum_cell.value().and_then(|&sum| value.map(|v| sum + v));

					let out = region.assign_advice(|| "out", config.advices[3], i, || out_score.ok_or(Error::Synthesis))?;
