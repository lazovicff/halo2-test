...
mod test {
	...
	impl<F: FieldExt> TestChip<F> {
		fn new() -> Self {
			TestChip { lhs: Some(F::from(1)), rhs: Some(F::from(1)) }
		}
	}
	...
	impl<F: FieldExt> Circuit<F> for TestChip<F> {
		...
		fn configure(meta: &mut ConstraintSystem<F>) -> TestConfig {
			...
			TestConfig { lhs_column, rhs_column, output }
		}
		...
		fn synthesize(
			...
			layouter.assign_region(
				...
				|mut region: Region<'_, F>| {
					...
					let c = a.value().and_then(|&a_val| b.value().map(|&b_val| a_val * b_val));
