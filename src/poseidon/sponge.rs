        ...
        for (i, chunk) in self.inputs.iter().enumerate() {
			...
			state = layouter.assign_region(
				...
				|mut region: Region<'_, F>| {
					...
					let poseidon_state = Self::load_state(config.poseidon_config.state, &mut region, round, perm_state.clone())?;

					let next_state = state.zip(poseidon_state).zip(config.state).try_map(|((prev_state, pos_state), column)| {
...
mod test {
	...
	use halo2_proofs::{
        circuit::{Layouter, SimpleFloorPlanner, Region, AssignedCell},
        dev::MockProver,
        pairing::bn256::Fr,
        plonk::{Circuit, Column, ConstraintSystem, Error, Instance, Advice},
    };
    ...
    impl Circuit<Fr> for PoseidonTester {
        ...
        fn without_witnesses(&self) -> Self {
            Self { inputs1: [None; 5], inputs2: [None; 5] }
        }
        ...
        fn synthesize(
			...
			let inputs1 = layouter.assign_region(
				|| "load_state1",
				|mut region: Region<'_, Fr>| Self::load_state(
					&config.sponge,
					&mut region,
					0,
					self.inputs1
            ...
            let result_state = poseidon_sponge.squeeze(&config.sponge, layouter.namespace(|| "poseidon_sponge"))?;
